  ...
  class Chain {
   public:
    ...
    Bucket &get_bucket_to_append() {
      if (buckets.empty() || buckets.back().n * 16 > BCAP) {
        buckets.push_back(Bucket(allocate_elements(BCAP), 0));
      }
      return buckets.back();
    }

    std::pair<Chain, Chain> split_chain() {
      assert(!buckets.empty());
      long long p =
          buckets[rng.nextInt(int(buckets.size()))].get_random_pivot();

      Chain left_chain, right_chain;
      for (int nth = 0, k = 0; nth < int(buckets.size()); nth++) {
        ...
        long long *z = b.arr + k;
        int n = min(b.n - k, min(BCAP - L.n, BCAP - R.n));
        k += n;
        for (int j = n; j--; z++) {
          int is_less = *z < p;
          *x = *z;
    ...
    std::pair<Chain, Chain> split_chain_nb() {
      assert(!buckets.empty());
      long long p =
          buckets[rng.nextInt(int(buckets.size()))].get_random_pivot();
