...
class Bucket {
 public:
  ...
  void moveToFromIdx(Bucket *to, int fromIdx) {
    ...
    assert(to->n + this->n - fromIdx <=
           BSIZE);  // make sure the receiver has enough space
    memmove(to->arr + to->n, arr + fromIdx,
            (this->n - fromIdx) * sizeof(long long));
    to->n += this->n - fromIdx;
    this->n = fromIdx;
  }
  ...
  int partition(long long const &v) {
    ...
    return int(std::partition(arr, arr + this->n,
                              [&](long long x) { return (x < v); }) -
               arr);
  }
};
...
void fusion(long long *Lp, long long *Rp, int *hi, int *lo, int &nhi,
            int &nlo) {
  ...
  if (!chain) {  // the root chain is empty.
    ...
    if (chain->slack()) {
      ...
      if (((Bucket *)chain->tail)->slack()) {
        b->moveToFromIdx(
            (Bucket *)chain->tail,
            b->size() - std::min(b->size(), ((Bucket *)chain->tail)->slack()));
      }
    }
    if (b->size()) {
      chain->add_chain(b);
